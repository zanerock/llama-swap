...
- ✅ llama-swap API
  ...
  - `/running` - list currently running models ([#61](https://github.com/mostlygeek/llama-swap/issues/61)). Each entry reports `state`, `since` (when the model entered `state`), `in_flight_requests` and `busy` (null unless `state` is `ready`). `busy` remains `true` for `busyGracePeriod` seconds after a model's last in-flight request completes, even once `in_flight_requests` returns to 0, so a caller polling between turns of a multi-turn conversation does not see a false idle reading between calls; a `busyGracePeriod` of 0 disables this and `busy` flips to `false` the instant `in_flight_requests` reaches 0. Read-only and cheap: listing never loads, unloads or health checks a model, so it is safe to poll.
